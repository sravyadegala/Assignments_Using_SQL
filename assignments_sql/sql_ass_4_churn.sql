...
select * 
from codebytes limit 100;

-- 2. Which months will you be able to calculate churn for.Check the range of months of data provided on subscription start.
select Max(subscription_start) from codebytes;
select min(subscription_start) from codebytes;
select  distinct date_format(subscription_start,'%m') as month
from codebytes where subscription_start between  "2016-12-01" and "2017-03-30" ; 

-- 3. You’ll be calculating the churn rate for both segments (87 and 30) over the first 3 months of 2017 
-- (you can’t calculate it for December, since there are no subscription_end values yet). To get started, 
-- create a temporary table of months that contains two columns: first-day and last-day values 
-- containing the first day and last day of the month.
with months as
(select 
...
-- 4. Create a temporary table called cross_join, from 
-- table codebytes and your months. Be sure to SELECT every column.
with months as
(select 
'2017-01-01' as 'first_day',
'2017-01-31' as 'last_day'
...
-- 7. Create a status_aggregate temporary table that is a SUM of 
-- the active and canceled subscriptions for each segment, for each month.
...
-- 8. Calculate the churn rates for the two segments over the
-- three month period. Which segment has a lower churn rate?
with months as
(select 
'2017-01-01' as 'first_day',
'2017-01-31' as 'last_day'
